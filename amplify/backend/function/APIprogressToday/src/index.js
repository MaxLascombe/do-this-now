    ...
    if (task.repeat !== 'No Repeat') {
      ...
      if (task.repeat === 'Daily') {
        ...
        if (task.repeatUnit === 'day') {
          ...
          if (
            task.repeatWeekdays &&
            Array.isArray(task.repeatWeekdays) &&
            task.repeatWeekdays.some(x => x)
          ) {
            // Count the number of selected weekdays
            const selectedWeekdaysCount = task.repeatWeekdays.filter(
              x => x
            ).length
...
exports.handler = async event => {
  ...
  const cappedTodo = Math.min(
    todo,
    Math.max(theoreticalMinimum + 60, 15.5 * 60)
  )
